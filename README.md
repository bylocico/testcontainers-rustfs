...
For example, `@bylocico/testcontainers-rustfs@1.0.0-beta.1` defaults to:
...
**Prereleases:** versions like `1.0.0-beta.1` are published with an explicit **dist-tag** (for example `beta`), not only `latest`. Consumers install them with:
...
5. Push to `main`.
6. Create a GitHub **Release** (publish event) for that version, for example tag `v1.0.0-beta.1`, or run the workflow manually from the Actions tab.
