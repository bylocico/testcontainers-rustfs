...
For example, `@bylocico/testcontainers-rustfs@1.0.0-rc.1` defaults to:
...
```ts
await new RustfsContainer('rustfs/rustfs:1.0.0-rc.1').start()
...
new RustfsContainer('rustfs/rustfs:1.0.0-rc.1')
```
...
```bash
node scripts/update-rustfs-version.mjs --version 1.0.0-rc.1
...
**Prereleases:** versions like `1.0.0-rc.1` are published with an explicit **dist-tag** (for example `beta`), not only `latest`. Consumers install them with:
...
1. Confirm the target RustFS Docker tag exists:
   ...
   ```bash
   docker manifest inspect rustfs/rustfs:1.0.0-rc.1
   ```
...
5. Push to `main`.
6. Create a GitHub **Release** (publish event) for that version, for example tag `v1.0.0-rc.1`, or run the workflow manually from the Actions tab.
